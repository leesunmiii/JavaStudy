 ...
 * 										=> 기술 면접 단골
 * 										=> 면접 (자바에 대한 질문)
 * 										=> 오버로딩을 설명할 때 new, add 라는 단어가 나와야함
 * 										=> 오버로딩을 하려면 메소드명이 동일해야함
 * 										=> 매개변수의 갯수나 데이터형이 다르다
 * 										=> 리턴형은 관계없다
 * 									A()
 * 									A(int a,int b) => A(int,int) 이렇게 저장된다
 * 									A(String s,double d) => A(String,double) 이렇게 저장된다
 * 									***A(int c,int d) => A(int,int)=> 오버로딩이 안된다
 * 									=> 실제 메소드에도 적용
 * 									*** void도 리턴형이다 (단지 결과값이 없을뿐)
 * 					기타
 ...
 * 			}
 * 			
 * 			리턴형 =>  요청에 대한 결과값 (1개만 사용가능)
 * 					=> 여러개인 경우 : 클래스/배열
 * 					=> 리턴형이 없는 경우 : void
 * 			기본형, 배열, 클래스를 리턴 할 수 있슴
 * 			메소드명 => 알파벳이나 한글로 시작
 ...
 * 					  공백이 있으면 안된다
 * 					  특수문자 사용 => (_.$)
 * 					  약속사항) 소문자로 시작한다, 두번째 단어의 첫 자는 대문자 사용
 * 					  *** 소프트웨어 이행
 * 						  ----------- 한글에 문제가 생긴다
 * 									  가급적이면 영문사용 권장
 * 	                                                    
 ...
 *  			A a=메소드명()
 *  
 *  		***** 메소드는 호술시마다 => 처음부터 return이 있는 곳까지
 *  								실행 => 호출된 위치로 다시 돌아간다
 *  
 *  		예)
 *  			void aaa()
