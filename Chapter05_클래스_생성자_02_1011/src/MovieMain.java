...
class MovieSystem{
	...
	MovieSystem() { //생성자
		...
		try
		{
			...
			// MSA => Spring Cloud 까지는 해야 취업 승산있슴
			StringBuffer data=new StringBuffer();
			int i=0; //char => fr.read() => 한글자씩 정수로 읽음
			while((i=fr.read())!=-1)
			{
				//-1 => EOF (문장 끝까지)
				data.append((char)i);
			...
			for(String ss:datas)
			{
				String[] s=ss.split("\\|");
				movies[i]=new Movie();
				//movies[i].rank=Integer.parseInt(ss[0]); // 문자열을 정수형으로 바꾸는 함수=>Integer.parseInt 
				movies[i].rank=s[0];
				movies[i].title=s[1];
				movies[i].genre=s[2];
				movies[i].poster=s[3];
...
//		System.out.println("상세 볼 영화 선택(1~1938):");
