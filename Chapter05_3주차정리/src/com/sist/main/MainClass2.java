 ...
 * 			=> 접근 범위 : 패키지 상관없이 모든 클래스에 접근가능
 * 
 * 		*** 크기 외우기
 * 		private < default < protected < public
 * 		-------------------------------------- 오버라이딩에 나옴 => 하나의 조건 중, 접근지정어의 축소는 불가능 확장은 가능
 * 	2. 관련된 클래스를 모아서 관리 => 찾기
 * 	   ---------------------- 패키지 (폴더 개념) => 클래스별 분리
 * 		패키지 명칭을 줄 때는 패키지 명 안에 키워드 사용하면 안된다
 * 		com(org).회사명.분리단위
 * 		--------------------
 * 		vo => 클래스형 데이터
 * 		dao => 데이터베이스 연동
 * 		model => 브라우저로 전송
 * 		manager => 크롤링, 파일 입출력
 * 		service => 데이터베이스 여러개를 한번에 처리할 때 (JOIN,SUBQUERY)
 * 		----------------------------------------------------------
 * 
 ...
 * 
 * 		*** 자바 소스 코딩의 형식
 * 			package => 한번만 사용가능 (import보다 package가 먼저와야함)
 * 			import => 여러개 사용가능
 ...
 * 				멤버메소드
 * 				{
 * 					변수선언 => 지역변수 => 메소드 호출이 종료되면 자동으로 사라짐
 * 				}
 * 				=> 가독성 => 지금 순서로 코딩
 * 			}
 * 
