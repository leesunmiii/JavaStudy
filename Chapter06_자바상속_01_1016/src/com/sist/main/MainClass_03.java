 ...
 * 		   -------
 * 			  < 목적 >
 * 			  데이터 손실 방지 : 캡슐화 (변수 : private => 은닉화할 때 사용, 가정(보이지않는다), 기능 => 처리)
 * 									--- 저장 / 읽기 => setter/getter
 * 									--- HTML(Jsoup) 
 * 			  재사용 : 상속 / 포함***
 * 			  수정 / 추가 : 유지보수
 ...
 * 						| 단일 상속만 가능, 키워드는 extends
 * 					---> 있는 그대로 사용 (변경x) => 포함
 * 					class Window
 * 					{
 * 						JFrame fr=new JFrame() => 포함 // 더 많이 사용됨
 * 					}
 * 					---> 변경해서 사용 => 상속
 * 					class Window extends JFrame
 * 					= new를 이용하지 않아도 메모리 할당이 가능(리플랙션)
 * 
 * 		사용법
 * 		class A
 * 		class C
 * 		class B extends A,C => 오류 (단일상속만 가능하기 때문)(C언어,React는 가능)
 *		-----------------------------------------------------------------
 *		class A
