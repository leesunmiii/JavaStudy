 ...
 * 			|				----------	명령문의 집합 : 메소드
 * 			|				---------- 	기능별 처리	
 * 		데이터 한 개 저장 : 변수 (지역 변수, 전역 변수 => 공유 변수, 객체 변수)
 * 							---------	------------ 클래스 안에서 사용
 ...
 * 						  메소드
 * 			= 분석 (가독성) => 수정(유지보수), 오류처리 
 * 			  기능별 분리 (예 : 로그인처리, 회원가입, 목록 출력, 페이지 나누기...)
 * 			= 다른 클래스와 연결 ('메세지'라고 함)
 ...
 * 				데이터 처리
 * 				오라클 닫기
 * 			= 재사용이 높다 (필요할때마다 저장하고 가져다 쓰고)
 * 			= 구조적인 프로그램 작성 => 단락을 나눠서 작업
 * 		2) 메소드 형식
 * 		   [접근지정어][옵션] 리턴형 메소드명 (매개변수...) : 선언부	
 * 			--------  -----
 * 			public(공개)   static : new를 사용하지 않고 자동으로 메소드에 저장
 * 			private		  abstract : 선언하는 메소드 => 추상 클래스 / 인터페이스를 공부해야 앎
 * 												  => 프로그램 설계할 때, 요구사항 분석 때 많이 사용
 ...
 * 			매개변수 : 사용자의 요청값 () 가로안에 설정하고 들어감
 * 					예) 로그인 요청
 * 						-------- id,pwd 받아야함// 사용자가 보내준 값을 매개변수로
 * 						아이디 찾기
 * 						-------- id // 아이디가 있다없다 결과값을 리턴형으로
 * 						검색
 * 						-------- 검색어
 * 						String substring(int start, int end)
 * 						------           -------------------
 * 						 리턴형				   매개변수
 * 						=> 일반적으로 : 변수가 3개 이상이면 묶어서 매개변수로 설정
 * 									  	   ------ 배열, 클래스
 * 			메소드명 : 식별자 (변수)
 * 					 = 알파벳이나 한글로 시작한다	
 * 					 = 메소드명의 길이 제한은 없다
 ...
 * 					X			     O
 * 				=> 메소드 자체에서 처리 => 출력
 * 				void main(String[] args) => 리턴형이 없는 경우에는 void를 앞에 붙여야함
 * 				void println(String s)
 * 		   -----------------------------------------
 * 					X	             X
 * 				System.out.println() => 다음줄에 출력 (거의 빈도수가 없다, 많이 사용하지 않는다)
 * 		   -----------------------------------------
 * 		4) 메소드 선언 및 호출 방법
 * 			public class A
 * 			{
