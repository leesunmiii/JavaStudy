 ...
 * 				=> 순서가 지정 (인덱스)
  			...
  			Set : HashSet
 				...
 				=> 데이터 중복 허용X
 					*** null 값 추가 가능 (한번만 가능)
 				=> 장르별, 부서별, 상품별로 가져온다던가 할 때 사용
 				=> List에서 중복데이터를 제거할 때 사용
 				=> 주요 메소드
 					1) 추가 : add()
 					2) 삭제 : remove(Object)
 					3) 존재여부 : isEmpty()
 					4) => 일반 for문 사용불가(인덱스가 없기 때문에)_ => for-each 사용
 					5) 갯수 : size()
  			Map : HashMap
 		...
 		사용법 <> => 클래스<데이터형> => 리턴형 / 매개변수가 변경
 						-------- 클래스형이 들어가야함
 		     단점 : 기본형 사용 불가능
 		     	   ----- 기본형을 사용할 수 있게 클래스형을 제공 : Wrapper
...
public class MainClass_Collection정리 {
	...
	public static void main(String[] args) {
		...
		try
		{
			...
			prop.load(new FileInputStream("C:\\Users\\SIST\\git\\java-study\\Chapter10_라이브러리(java.util)_collection_1026\\src\\com\\sist\\lib\\student.properties"));
