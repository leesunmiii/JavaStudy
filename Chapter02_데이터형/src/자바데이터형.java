 ...
 * 		1) 프로그램의 기본
 * 		= 데이터 저장하는 방법
 * 		= 데이터를 한 개만 저장 : 변수 / 상수, 리터럴 : 실제 저장할 값
 * 		                    ---- 메모리 저장 공간의 별칭
 * 								---- 주소 (0~...)
 * 		  자바의 메모리 구조
 * 		  ----------------
 ...
 *  	  ----------------
 *  
 *  	  프로그램 => 데아터 관리 (Vue, React => 데이터 관리)
 *        실제 메모리 (RAM) => 휘발성 메모리 => 프로그램이 종료하면 저장된 데이터가 사라짐.
 *                 100을 저장
 *        0	byte ------------ 메모리는 4byte씩 나누어진다
 *        	         사용중
 *        4 byte ------------
 *                   사용중
 *        8 byte ------------
 ...
 *        	 정수형
 *        		byte => 1 byte (8bit) => 0,1을 8개 저장하는 공간
 *                 -----------------------------------
 *                 ㅣ 부호 (양수(0), 음수(1)를 결정하는) ㅣ  1 ㅣ 1  ㅣ 1  ㅣ  1 ㅣ 1  ㅣ 1  ㅣ 1  ㅣ
 *                 ------------------------------------
 *                 
 *                 01111111
 *                 --------
