...
class MusicData{
	...
	// {구현} => 초기화 블록 => 자동 생성
	{
		...
		try
		{
			...
			String data="";
			int i=0; // 1글자씩 읽어옴 => 정수형으로 읽어옴 => 저장할 때는 char로 저장하고 받을 때는 int로 받아옴
			while((i=fr.read())!=-1)
			{
				// -1파일의 끝 =>EOF (end of file)
				data+=(char)i;
			}
			fr.close(); // 파일 닫기
...
public class MusicMain_초기화블록 {
	...
	public static void main(String[] args) {
		// TODO Auto-generated method stub
			...
			System.out.print("동영상 선택:(1~49=>47번 제외):");
			int mno=scan.nextInt();
			Music mm=md.musics[mno-1]; //인덱스 0부터 시작하기 때문에
