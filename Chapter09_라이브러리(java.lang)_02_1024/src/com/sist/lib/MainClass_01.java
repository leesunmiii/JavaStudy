 ...
 * 												   --------- 확장 불가능 (있는 그대로 사용해야함)
 		...
 		=> Object : 최상위 클래스 => 모든 클래스의 상위 클래스
 					(사용자정의, 라이브러리 => 모든 클래스는 Object로부터 상속을 받는다)
 					= ***toString() : 객체를 문자열화
 								   기본은 메모리 주소 출력
 								   => 오버라이딩 (멤버변수 값 확인할 때)
 					= ***clone() : 메모리 주소를 복제해서 새로운 메모리를 생성
 								예) 게임에 아바타 만들기 => 디자인 패턴, 알고리즘
 													   ----------------- 스프링 전에 배울 예정
 					= finalize : 소멸자 (자동 호출), 메모리 해제 시 자동호출
 		=> System : 출력, 메모리 해제, 프로그램 종료
 					= *** println() : 에러 출력
 									  멤버변수 값 확인
 									  전송 값 확인
 									  윈도우/브라우저 => 자체 출력하기 때문에 디버깅 할 때 많이 사용
 					= gc() : 가비지 컬렉션을 호출해서 메모리를 비워주는 역할
 							멀티미디어, 애니메이션
 							=> 실시간 도로 cctv
 					= exit() : 종료
 						  --- 0(정상), 비정상
...
 						--------------
		...
		3. 두개의 String에 값만을 비교해서 같으면 true, 다르면 false를 반환한다.(대소비교)
				...
				*** public boolean equals (String s)
					=> 웹 : 로그인, 아이디 찾기, 아이디 중복
					  --- 웹 공부 시작하면 가장 먼저 배울건 : CRUD (게시판) => 회원가입 => 로그인 처리 
					  		=> 목록(페이지나누기) => 상세보기(댓글) => 최신방문 (쿠키), 로그인상태유지(세션)
					=> 대소문자 구분해서 비교
		...
		6. 문자열의 길이를 반환한다.
			...
			=> 사용처 : 비밀번호 / 글쓰기
								----- 오라클의 지정된 문자 갯수 초과
									=> 오라클은 문자의 갯수 지정
									=> title VARCHAR2(1000) => 한글은 300글자만 사용가능
										=> 한글은 3byte
