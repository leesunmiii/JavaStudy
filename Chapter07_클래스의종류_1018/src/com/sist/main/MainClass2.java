 ...
 * 				  ======> ~VO (Value Object) 값만 가지고 있는 메소드,, getter/setter만 가지고 있는 => Spring
 * 				 		  ~DTO (Data Transfor Object) 데이터를 묶어서 전송하는 메소드 => MyBatis
 * 						   => 웹 => JSP (~Bean) => JSP
 * 							Music(X) => MusicVO
 ...
 * 		1) 추상클래스 => 단일 상속
 * 			= 주목적
 * 				관련된 클래스를 모아서 관리 (서로 다른 클래스를 연결해서 사용하는 기법)
 * 			= 미완성 클래스 (메소드를 구현하지않는 경우) => 선언
 * 				=> new를 이용해서 메모리 할당을 할 수 없다
 * 				=> 사용시에는 반드시 상속을 내려서 구현 후에 사용
 * 								--------------- 오버라이딩 기법
 *			= 단일 상속만 가능
 *			= 선언만 한 메소드가 존재할 수 있다
 *			= 인스턴스 메소드, 인스턴스 변수 사용가능
 *			= 형식)
 ...
 * 				묻고 답하기 : 목록 출력, 검색, 글쓰기, 내용보기, 수정하기, 삭제하기, 답변하기
 * 				댓글 게시판 : 목록 출력, 검색, 글쓰기, 내용보기, 수정하기, 삭제하기, 댓글
 * 				후기 게시판 : 목록 출력, 검색, 글쓰기, 내용보기, 수정하기, 삭제하기, 업로드
 * 
 
 * 				1) 공통으로 사용되는 메소드를 찾는다
 *						=> 문제풀이 
 * 							
 * 				2) 추상메소드를 마든다 (구현이 안된 메소드)
 * 					=> 클래스를 묶어서 한개의 이름을 제어	
 * 					=> 설계 => 프로그램에 맞게 구현
 * 						      ---------------- 
 * 					=> 사이트를 만들어라
 * 					=> 이런 사이트를 만드는데 이런 기능이 존제ㅐ한다
 * 					=> 상속받아서 구현
 * 					=> 추상클래스 => 모든 게시판 제어 => 카페
 * 				=> 추상 클래스 확인
 * 					1) 선언된 클래스와 생성자가 다르다
